...
export const ValidateInput = (field:IField, inputsObj:IState) => {
  ...
  if (value?.trim()?.length < 1) {
    ...
    if(value.length < 8) validatedInputs[name].error = 'Password must be atleast 6 characters'
   else if(!isPasswordStrong(value)) validatedInputs[name].error = 'use atleast a uppercase,lowercase,number and special character'
   else validatedInputs[name].error = ''
  }
